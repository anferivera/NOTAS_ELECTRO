...
Uno de los problemas fundamentales de la dinámica que ha intrigado el pensamiento del hombre desde los amaneceres de la civilización, ha sido el movimiento de los cuerpos celestes. Es este pensamiento justamente el que impulsó el desarrollo de la ciencia hasta nuestros días. 

\subsection{Modelos planetarios}
\begin{enumerate}
\item \textbf{Modelo geocéntrico:} Este primer modelo formulado en el año $100$ por el astrónomo griego Ptolomeo de Alejandría, planteó que la tierra era el centro del universo y el conjunto de las estrellas fijas inundaban el universo circundante alrededor de la tierra. Las estrellas errantes que se observaban en la noches, giraban en torno a la tierra, algunos describiendo trayectorias muy complicadas, conocidas como epiciclos \cite{alonso-z}. Cabe anotar que este modelo tenía como fundamentación al ``hombre como el centro del universo''.
...
Cabe anotar que la idea de tener al sol como centro del universo, no era una idea nueva. Se tienen datos históricos que argumentan que fue realmente una idea del filósofo Aristarco de Samos, muchos años atrás.

\end{enumerate}

Una vez establecido este último modelo, el físico Johanes Kepler ($1571-1630$) descubre las leyes planetarias haciendo un cuidados análisis de los datos coleccionados por el astrónomo Tycho Brahe ($1546-1601$), el cual murió sin poder dar una buena interpretación de sus datos, coleccionados durante toda su vida.\\
Dichas leyes establecidas por Kepler fueron empíricas y su soporte y explicación teórica fue dada unos años después por el Físico Isacc Newton ($1642-1727$). Este último hizo la contribución más notable a la dinámica de los cuerpos celestes, la cual tiene su soporte más importante en lo que hoy conocemos como la ley de la gravitación universal, formulada en el año $1686$ y publicada en el año de $1687$ en el famoso libro; "Los Principia" (Philosophiae Naturalis Mathematica).
...
\item \textbf{LEY $1$}: Todos los planetas se mueven en órbitas elípticas con el sol en una de sus focos. 


\begin{figure}
\begin{center}
\includegraphics[scale=0.4]{gravitacion/ley1}[h]
\end{center}
\label{ley1}
\caption{Primera ley de Kepler}
\end{figure}

Dicha ley generó una gran controversia para la época, debido a que las órbitas propuestas por el modelo heliocéntrico eran circulares. Las órbitas circulares reflejaban la perfección del cielo. Las órbitas elípticas desafiaban la nueva concepción física y había obstaculizado el trabajo del señor Tycho durante toda su vida.\\
Cabe anotar que dicha ley es un resultado de las estructura funcional de la fuerza de gravitación universal que estudiaremos más adelante.

\item \textbf{LEY $2$}: El radio-vector dirigido desde el sol a un planeta, barre áreas iguales en tiempos iguales. 

\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.4]{gravitacion/ley2}
\end{center}
\caption{Segunda ley de Kepler}
\label{ley2}
\end{figure}

Kepler obtuvo dicha ley estudiando detenidamente los datos obtenido en las observaciones astronómicas de Tycho. En la figura \ref{ley2} podemos ver que el área barrida en un tiempo $t$ fijo, siempre es $A$. 

Veamos las primeras implicaciones de esta ley:
%Consideremos un diferencial de área $dA$
\begin{equation}
dA \approx \dfrac{|\vec{r} \times d\vec{r}|}{2} 
\Rightarrow \dfrac{dA}{dt} \approx \dfrac{1}{2m}|\vec{r} \times m \dfrac{d\vec{r}}{dt}| = \dfrac{1}{2m}|\vec{r}\times \vec{P}|=\dfrac{|\vec{L}|}{2m}
\end{equation}

Así, de el razonamiento anterior queda claro que el hecho de que se barran áreas iguales en tiempos iguales tiene como consecuencia que el momento angular $\vec{L}$ de los planetas alrededor del sol es un constante, es decir:

\begin{equation}
\vec{L}=\vec{cte}
\end{equation}

Además, esto a su vez establece uno de los precedentes fundamentales en la formulación de la ley de la gravitación universal que establecería Newton unos años después. El hecho de que el momento angular sea una constante implica que la fuerza que está actuando sobre los planetas, es una fuerza central, esto puede verse rápidamente:
...
Así, el torque neto $\vec{\tau}$ que actúa sobre cada planeta, debido a su interacción con el sol, es:

\begin{equation}
\vec{\tau}= \dfrac{d\vec{L}}{dt}= \vec{r}\times \dfrac{d\vec{p}}{dt} = \vec{r} \times \vec{F} ,
\end{equation}

donde $\vec{F}$ es la fuerza neta que actúa sobre cada planeta. Ahora, si $\vec{L}$ es una constante, entonces la ecuación anterior implica que:

\begin{equation}
\vec{r}\times \vec{F}= \vec{0} ,
\end{equation} 

es esto se deduce que $\vec{F}$ y $\vec{r}$ son antiparalelas (fuerza atractiva), es decir, \textit{la fuerza que mantiene en órbita a los planetas alrededor del sol, es una fuerza central atractiva}.

Para dos puntos $1$ y $2$ sobre la trayectoria elíptica de cada planeta, se tiene:
\begin{equation}
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.8]{gravitacion/perihelio}
\end{center}
\label{elipse}
\caption{Movimiento elíptico de los planetas}
\end{figure}

En el caso particular del perihelio y el afelio \ref{elipse} :
...
Resumimos esta ley, afirmando que es una consecuencia directa de la conservación del momento angular en un sistema bajo fuerzas centrales. Resultado conocido en la mecánica de una partícula.
...
De acuerdo a la figura \ref{elipse}:
...
\vspace{0.5 cm}
%ejercicio2
\item Dos estrellas de masas $m$ y $M$ separadas por una distancia $d$, giran en  órbita circular alrededor de su centro de masa CM, tal como se muestra en la parte izquierda de la Figura~\ref{fig:sistema-binario-Mm}. 
%
\begin{figure}[h]
\begin{center}
...
%
\item El sistema binario Plaskett consiste de dos estrellas de masas iguales girando en torno a su centro de masa CM, tal como se muestra en la parte derecha de la Figura~\ref{fig:sistema-binario-Mm}. 
...
\end{align}
%
Ahora, si multiplicamos la ecuación~\ref{ec2:LGU-eje2} la masa $m$ y la ecuación~\ref{ec3:LGU-eje2} la masa $M$ y las sumamos, tenemos la siguiente ecuación:
%
\begin{align}
\label{ec4:LGU-eje2}
...
\end{align}
%
Despejando la fuerza $F$ de la ecuación~\ref{ec1:LGU-eje2} y reemplazando en la ecuación~\ref{ec4:LGU-eje2}, tenemos que:
%
\begin{align}
\label{ec5:LGU-eje2}
...
\end{align}
%
Finalmente, usamos la definición del periodo en un movimiento circular uniforme $P=\dfrac{2\pi}{\omega}$ y la relación $r_1+r_2=d$ mostrada en la Figura~\ref{fig:sistema-binario-Mm}, para concluir que:
%
\begin{align}
\label{ec6:LGU-eje2}
...
\item Si asumimos que $M\gg m$ en la ecuación~\ref{ec5:LGU-eje2}, entonces:
%
\begin{align}
\label{ec7:LGU-eje2}
...
\item Usando la ecuación~\ref{ec6:LGU-eje2} con $M=m$, tenemos que:
%
\begin{align}
\label{ec8:LGU-eje2}
...
cambiando su velocidad en el perigeo, tal como se muestra en la Figura~\ref{fig:cambio-orbita}. 
...
\end{align}
%
Análogamente, en la órbita circular, la energía mecánica también está dada por la ecuación~\ref{ec1:eje3}, donde ahora el semieje mayor de la órbita debe ser interpretado como la distancia que hay al perigeo, es decir:
%
\begin{align}
\label{ec4:eje3}
...
\end{align}
%
donde tuvimos en cuenta que la rapidez $v_2$ tiene la misma estructura de la ecuación~\ref{ec8:eje3} cambiando $E_1$ por la energía $E_2$. De este resultado se puede concluir que el satélite debe disminuir su velocidad para poder pasar desde la órbita elíptica a a la órbita circular.
...
Para el caso general de una distribución de masa volumétrica (Figura \ref{fig_distribucion_continuo} ), superficial o lineal; el campo gravitacional se calcula con la siguiente expresión:
...
donde la integral debe hacerse sobre todo el cuerpo de masa $M$. Note que que el la ecuación anterior, el vector unitario $\hat{u}_r$ y la magnitud $r$ son variables al igual que $dm$, tal como se muestra en la Figura \ref{fig_distribucion_continuo}. 
...
Dado que la fuerza gracitacional $\vec{F}_g$ es una fuerza central y sólo depende de la distancia $r$ entre las dos masas (Figura \ref{energia potencial} ), entonces $\vec{F}_g$ es una fuerza conservativa y podemos asociarle una función energía potencial gravitacional $E_p(r)$.
...
\end{equation}
\item Para tres masas puntuales $m_1$, $m_2$, $m_3$, separadas distancias $r_{12}$, $r_{13}$, $r_{23}$ (ver Figura \ref{figtresmasas} para mayor ilustración):
\begin{eqnarray}
\nonumber
...
\subsection*{Ejemplo \cite{klepner}}
Un proyectil de masa $m$ es lanzado desde la superficie de la tierra formando un ángulo $\alpha$ con la vertical tal como se muestra en la Figura \ref{fig-tiro}. Si la rapidez inicial del proyectil es $v_0=\sqrt{\dfrac{GM}{R_e}}$, donde $M$ es la masa de la tierra, determinemos la altura máxima alcanzada por el proyectil (\textbf{$r_{max}$}).
...
En cualquier campo $P$ (Figura \ref{camposuperposicion} ), el campo debido a un grupo de masas $m_1, m_2, \cdots, m_n$, es igual al vector suma de los campos $\vec{g}_i$ generados por cada una de masas individuales, es decir, el campo en el punto $P$ es la superposición de los campo individuales $\vec{g}_i$:
...
\item Una barra delgada de longitud $l$ tiene una masa $m$ uniformemente distribuida (Figura \ref{figbarra1}). El campo en el punto $P$ a una distancia $d$ lo largo del eje de la barra está dado por:
...
\item Una barra delgada de longitud $l$ tiene una masa $m$ uniformemente distribuida (Figura \ref{figbarra2}). El campo en el punto $P$ está dado por:
...
\item Caso general: Campo en el punto $P$ mostrado en la Figura \ref{figbarra3}.
...
Un anillo de radio $R$ tiene una masa $m$ uniformemente distribuida. Calculemos el campo $\vec{g}$ en un punto $P$ a lo largo del eje del anillo, tal como se muestra en la Figura \ref{fanillo}.
...
Considere un disco hueco de radio exterior $R_2$, radio interior $R_1$ y masa $M$ uniformemente distribuida. Calculemos el campo gravitacional en el punto $P$ mostrado en la Figura \ref{arandela}. Analice el caso cuando $R_1 \rightarrow 0$ (disco de radio $R_2$). Analice el caso cuando $R_1 \rightarrow 0$ y $z >> R_2$.
...
\begin{enumerate}
\item Para $r > R$ (exterior del cascarón esférico). De acuerdo a la Figura \ref{figesfera}
...
\item Para $r < R$ (intrior del cascarón esférico). De acuerdo a la Figura \ref{figesfera}
...
\begin{enumerate}
\item Para $r > R$ (exterior de la esfera), podemos integrar en cascarones esféricos y usar el resultado obtenido para el campo generado por un cascarón de masa diferencial $dm$ en el punto $P$ (ecuación \ref{ecucascaron}).
...
\item Para $r < R$ (interior de la esfera) sólo contribuye la masa $m'$ localizada en la esfera de radio $r < R$ tal como se muestra en la Figura \ref{figesfera2} , así:
...
Consideremos el anillo de masa $m$ uniformemente distribuida y radio $R$ mostrado en la Figura \ref{fanillo}. Calculemos el potencial gravitacional en el punto $P$.
...
Calculemos el potencial gravitacional de una arandela de radio exterior $R_2$ y radio interior $R_1$ en el punto $P$ mostrado en la Figura \ref{arandela}.
